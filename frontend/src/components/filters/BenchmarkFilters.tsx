    ...
    <div className="space-y-6 p-6 bg-card rounded-lg border">
      ...
      <div className="space-y-2">
        ...
        >
          <SelectTrigger id="binary-select">
            <SelectValue placeholder={isLoadingBinaries ? "Loading..." : "Select binary"} />
          </SelectTrigger>
          <SelectContent>
            {binaries.map((binary) => (
              <SelectItem key={binary.id} value={binary.id}>
                <div>
                  ...
                  {binary.description && (
                    <div className="text-sm text-muted-foreground">{binary.description}</div>
                  )}
                </div>
              </SelectItem>
            ))}
          ...
          <SelectTrigger id="environment-select">
            <SelectValue 
              placeholder={
                ...
                !selectedBinaryId ? "Select binary first" : 
                "Select environment"
              } 
            />
          </SelectTrigger>
      ...
      <div className="space-y-2">
        ...
        <Select 
          ...
          onValueChange={(value) => onPythonVersionChange(value === "all" ? null : value)}
        >
          ...
          <div className="max-h-64 overflow-y-auto space-y-2">
            {filteredBenchmarks.length === 0 ? (
              <div className="text-sm text-muted-foreground">
                {availableBenchmarks.length === 0 ? "No benchmarks available" : "No benchmarks match your search"}
              </div>
            ) : (
