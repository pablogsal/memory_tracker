  ...
  return useMemo(() => {
    ...
    const chartData: ChartDataPoint[] = sortedDates.map((timestamp) => {
      ...
      selectedBenchmarks.forEach((benchmarkName) => {
        ...
        const dataPoint = benchmarkTrends.find((point) => point.timestamp === timestamp);
...
export function useYAxisDomain(chartData: ChartDataPoint[], selectedBenchmarks: string[]) {
  return useMemo(() => {
...
export function useBuildComparisonData(
  ...
  return useMemo(() => {
    if (!allTrendData || selectedBenchmarks.length === 0 || selectedBinaries.length === 0) {
      return [];
    }
    ...
    selectedBenchmarks.forEach((benchmark) => {
      ...
      selectedBinaries.forEach((binary) => {
        ...
        const values = limitedData.map(point => point.high_watermark_bytes).filter(v => v > 0);
