...
export const queryKeys = {
  ...
  environmentsForBinary: (binaryId: string) => ["environmentsForBinary", binaryId] as const,
  ...
  batchBenchmarkTrends: (queries: Array<{
    benchmark_name: string;
    binary_id: string;
    environment_id: string;
    limit: number;
...
export function useCommitsForBinaryEnvironment(
  ...
  return useQuery({
    queryKey: queryKeys.commitsForBinaryEnvironment(binaryId || "", environmentId || ""),
    queryFn: () => api.getCommitsForBinaryAndEnvironment(binaryId!, environmentId!),
...
export function useBenchmarkTrends(params: {
  benchmark_name: string;
  binary_id: string;
  environment_id: string;
  limit: number;
  ...
  return useQuery({
    queryKey: params ? queryKeys.benchmarkTrends(params) : ["benchmarkTrends", "disabled"],
...
export function useBatchBenchmarkTrends(
  ...
  return useQueries({
    queries: queries.map((query) => ({
      ...
      staleTime: 1 * 60 * 1000,
      enabled: !!query.benchmark_name && !!query.binary_id && !!query.environment_id,
    })),
  });
}
