...
import { ArrowLeft, Settings, Code2, AlertCircle, Zap, Bug, Gauge, Shield, Search, ChevronDown, ChevronRight, Server, GitCommit, Calendar, User, Hash, ExternalLink } from 'lucide-react';
...
function EnvironmentCard({ binary, binaryId, environment, commits, isLoadingCommits, onLoadCommits }: EnvironmentCardProps) {
              ...
              {isLoadingCommits ? (
                          ...
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <code className="font-mono text-sm font-semibold">{commit.sha.slice(0, 7)}</code>
                                <Badge variant="outline" className="text-xs">
                                  py {commit.python_version.major}.{commit.python_version.minor}.{commit.python_version.patch}
                                </Badge>
                              </div>
                              ...
                              <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                                ...
                                <span className="flex items-center gap-1">
                                  ...
                                  {format(new Date(commit.run_timestamp), 'MMM d, yyyy HH:mm')}
                                </span>
                              </div>
                            </div>
                          </div>
                          <div className="flex flex-col gap-2">
                            <Link href={`/diff?commit_sha=${commit.sha}&binary_id=${binaryId}&environment_id=${environment.id}`}>
                ...
                <Card className="border-dashed">
                  <CardContent className="flex items-center justify-center h-24 text-muted-foreground">
                    <p className="text-sm">No commits found for this binary + environment combination</p>
                  </CardContent>
                </Card>
              )}
            </div>
...
export default function BinaryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadBinaryAndEnvironments() {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to load binary details');
      } finally {
        setLoading(false);
      }
    }
  ...
  const loadCommitsForEnvironment = async (environmentId: string) => {
    ...
    try {
      ...
      const commits = await api.getCommitsForBinaryAndEnvironment(resolvedParams.id, environmentId);
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Binary Configuration Details</p>
          </div>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div>
                <h4 className="text-sm font-semibold mb-3 text-muted-foreground">DESCRIPTION</h4>
                <p className="text-sm leading-relaxed">{info.description}</p>
              </div>
              
              <div>
                <h4 className="text-sm font-semibold mb-3 text-muted-foreground">IDENTIFIER</h4>
              ...
              <div>
                <h4 className="text-sm font-semibold mb-3 text-muted-foreground">COMPILATION FLAGS</h4>
            ...
            <div className="space-y-6">
              <div>
                <h4 className="text-sm font-semibold mb-3 text-muted-foreground">TESTING OVERVIEW</h4>
                <div className="grid grid-cols-2 gap-4">
                  <Card className="p-4 text-center">
                    <div className="text-2xl font-bold text-primary">{environments.length}</div>
                    <div className="text-xs text-muted-foreground">Environments</div>
                  </Card>
                  <Card className="p-4 text-center">
                    <div className="text-2xl font-bold text-primary">
                      {environments.reduce((sum, env) => sum + env.commit_count, 0)}
                    </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Explore commits and benchmark results for this binary configuration across different environments.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Card className="border-dashed">
              <CardContent className="flex items-center justify-center h-32 text-muted-foreground">
                <div className="text-center">
                  ...
                  <p className="text-xs mt-1">Run some benchmarks to see data here</p>
                </div>
              </CardContent>
            </Card>
          )}
