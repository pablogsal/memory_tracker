...
function DiffTableContent() {
    ...
    async function loadData() {
      try {
        ...
        const [commitsData, binariesData, environmentsData] = await Promise.all(
          [api.getCommits(0, 100), api.getBinaries(), api.getEnvironments()]
        );
  ...
  useEffect(() => {
    async function updateAvailableEnvironments() {
      ...
      try {
        const environmentsForBinary =
          await api.getEnvironmentsForBinary(selectedBinaryId);
  ...
  useEffect(() => {
    async function updateAvailableCommits() {
      ...
      try {
        const commitsForBinaryAndEnv =
          await api.getCommitsForBinaryAndEnvironment(
            selectedBinaryId,
            selectedEnvironmentId
          );
