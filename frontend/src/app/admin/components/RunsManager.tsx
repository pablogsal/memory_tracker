...
export default function RunsManager() {
  ...
  const handleDelete = async (run: Run) => {
    if (!confirm(`Are you sure you want to delete run "${run.run_id}"? This will also delete all associated benchmark results.`)) return;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">View and manage benchmark execution runs</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleFilterChange('commit_sha', e.target.value)}
              />
            </div>
            
            <div>
              <Label htmlFor="binary-filter">Binary</Label>
              <Select value={filters.binary_id || 'all'} onValueChange={(value) => handleFilterChange('binary_id', value)}>
                <SelectTrigger>
            ...
            <div>
              ...
              <Select value={filters.environment_id || 'all'} onValueChange={(value) => handleFilterChange('environment_id', value)}>
                <SelectTrigger>
            ...
            <Card key={run.run_id} className="hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <CardDescription>
                        Commit: {run.commit_sha.substring(0, 8)}... by {run.commit.author}
                      </CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Badge variant="secondary">
                      Python {run.python_major}.{run.python_minor}.{run.python_patch}
                    </Badge>
                    <Button 
                      variant="ghost" 
                      size="sm"
              ...
              <CardContent>
                ...
                <div className="mb-4 p-3 bg-muted rounded-lg border">
                  ...
                  <div className="space-y-2">
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Full SHA</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-1">
                          ...
                          <p className="text-sm">
                            {format(new Date(run.commit.timestamp), 'MMM dd, yyyy HH:mm')}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Binary</p>
                    <p className="text-sm">{getBinaryName(run.binary_id)}</p>
                    <p className="text-xs text-muted-foreground">{run.binary_id}</p>
                  </div>
                  
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Environment</p>
                    <p className="text-sm">{getEnvironmentName(run.environment_id)}</p>
                    <p className="text-xs text-muted-foreground">{run.environment_id}</p>
                  </div>
      ...
      {runs.length > 0 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            Showing {(currentPage * pageSize) + 1} to {Math.min((currentPage + 1) * pageSize, pagination.total)} of {pagination.total} runs
          </p>
