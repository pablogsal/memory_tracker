...
export default function BinariesManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const binaryData = {
      id: formData.id,
      name: formData.name,
      flags: formData.flags.split(',').map(f => f.trim()).filter(f => f),
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Success",
          description: `Binary ${editingBinary ? 'updated' : 'created'} successfully`,
        });
      } else {
      ...
      toast({
        title: "Error",
        description: `Failed to ${editingBinary ? 'update' : 'create'} binary: ${error.message}`,
        variant: "destructive",
      });
    }
  };
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Comma-separated list of build flags</p>
              </div>
            ...
            <Card key={binary.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                <div className="space-y-3">
                  {binary.description && (
                    <p className="text-sm text-gray-600">{binary.description}</p>
                  )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {binary.flags.length > 0 ? (
                        ...
                        <span className="text-xs text-gray-500">No flags specified</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {!loading && binaries.length === 0 && (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-gray-900">No binaries found</h3>
            <p className="text-gray-500">Get started by creating your first binary configuration.</p>
