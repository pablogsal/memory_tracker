...
export default function EnvironmentsManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Success",
          description: `Environment ${editingEnvironment ? 'updated' : 'created'} successfully`,
        });
      } else {
      ...
      toast({
        title: "Error",
        description: `Failed to ${editingEnvironment ? 'update' : 'create'} environment: ${error.message}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleDelete = async (environment: Environment) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/admin/environments/${environment.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Manage benchmark execution environments</p>
        </div>
      ...
      {!loading && environments.length === 0 && (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-gray-900">No environments found</h3>
            <p className="text-gray-500">Get started by creating your first test environment.</p>
