...
async def github_auth_callback(
    ...
    try:
        print(f"🔄 Starting OAuth callback for code: {code[:8]}...")
        ...
        print(f"✅ Got user info for: {github_user.login}")
        
        # Check if user is admin
        is_admin = await github_oauth.is_admin_user(github_user.login, db)
        print(f"🔐 Admin check result for {github_user.login}: {is_admin}")
        
        if not is_admin:
            print(f"❌ User {github_user.login} is not authorized as admin")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="User is not authorized as admin",
            )
        
        print(f"✅ User {github_user.login} is authorized, creating session...")
        
        # Create admin session
        try:
            session_token = await create_admin_session(db, github_user)
            print(f"✅ Created admin session for {github_user.login}")
        ...
        print(f"✅ Auth callback completed successfully for {github_user.login}")
