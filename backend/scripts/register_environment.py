...
async def register_environment(environment_id: str, name: str, description: str = None) -> bool:
    """Register a new environment configuration."""
    async with AsyncSessionLocal() as db:
        try:
            # Check if environment already exists
            existing_env = await crud.get_environment_by_id(db, environment_id=environment_id)
            if existing_env:
...
"""
    ...
    if args.list:
        ...
        asyncio.run(register_environment(args.environment_id, args.name, args.description))
    else:
        parser.print_help()
        sys.exit(1)
