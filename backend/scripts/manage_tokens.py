...
async def create_token(name: str, description: str = None) -> str:
    ...
    async with db_manager.AsyncSession() as db:
        try:
            ...
            print("   - Use this token with the worker via --auth-token or MEMORY_TRACKER_TOKEN environment variable")
...
async def list_tokens() -> None:
    ...
    async with db_manager.AsyncSession() as db:
        try:
            ...
            print("-" * 80)
            print(f"{'ID':<4} {'Name':<20} {'Status':<8} {'Created':<20} {'Last Used':<20}")
            print("-" * 80)
            
            for token in tokens:
                status = "Active" if token.is_active else "Inactive"
                last_used = token.last_used.strftime("%Y-%m-%d %H:%M:%S") if token.last_used else "Never"
                created = token.created_at.strftime("%Y-%m-%d %H:%M:%S")
                
                print(f"{token.id:<4} {token.name[:20]:<20} {status:<8} {created:<20} {last_used:<20}")
...
async def update_token_info(token_id: int, name: str = None, description: str = None) -> None:
...
async def search_tokens(name_pattern: str = None, description_pattern: str = None, 
                       active_only: bool = False, inactive_only: bool = False) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if description_pattern:
                conditions.append(models.AuthToken.description.ilike(f"%{description_pattern}%"))
            ...
            print(f"Found {len(tokens)} token(s) matching: {', '.join(criteria) if criteria else 'all tokens'}")
            print("-" * 80)
            print(f"{'ID':<4} {'Name':<20} {'Status':<8} {'Created':<20} {'Last Used':<20}")
            print("-" * 80)
...
async def show_token_details(token_id: int) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            print(f"Last Used: {token.last_used.strftime('%Y-%m-%d %H:%M:%S UTC') if token.last_used else 'Never'}")
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            active_result = await db.execute(
                select(func.count(models.AuthToken.id)).where(models.AuthToken.is_active == True)
            )
            active_tokens = active_result.scalar()
            
            used_result = await db.execute(
                select(func.count(models.AuthToken.id)).where(models.AuthToken.last_used.is_not(None))
            )
            ...
            newest_result = await db.execute(
                select(models.AuthToken).order_by(desc(models.AuthToken.created_at)).limit(1)
            )
            ...
            if oldest_token:
                print(f"Oldest Token: '{oldest_token.name}' (created {oldest_token.created_at.strftime('%Y-%m-%d')})")
            
            if newest_token:
                print(f"Newest Token: '{newest_token.name}' (created {newest_token.created_at.strftime('%Y-%m-%d')})")
            
            if most_recent_token:
                print(f"Most Recently Used: '{most_recent_token.name}' (used {most_recent_token.last_used.strftime('%Y-%m-%d %H:%M:%S')})")
...
async def cleanup_old_tokens(days: int = 90, dry_run: bool = True) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            tokens_to_cleanup = {token.id: token for token in old_unused_tokens + old_inactive_tokens}
            
            if not tokens_to_cleanup:
                print(f"No tokens found for cleanup (older than {days} days and unused/inactive)")
                return
            
            print(f"Found {len(tokens_to_cleanup)} token(s) for cleanup:")
            print("-" * 60)
            for token in tokens_to_cleanup.values():
                status = "Never Used" if not token.last_used else f"Inactive since {token.last_used.strftime('%Y-%m-%d')}"
            ...
            if dry_run:
                ...
                print()
                confirm = input("❗ Are you sure you want to delete these tokens? (yes/no): ")
...
async def export_tokens(format_type: str = "json", include_inactive: bool = True) -> None:
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if format_type == "json":
                ...
                print("id,name,description,is_active,created_at,last_used,token_preview")
                for token in token_data:
                    description = (token["description"] or "").replace('"', '""')
                    print(f'{token["id"]},"{token["name"]}","{description}",{token["is_active"]},'
...
"""
    ...
    create_parser = subparsers.add_parser('create', help='Create a new authentication token')
    create_parser.add_argument('name', help='Human-readable name for the token (e.g., "CI System", "John Doe")')
    create_parser.add_argument('--description', '-d', help='Optional description of the token purpose')
    
    # List tokens command
    list_parser = subparsers.add_parser('list', help='List all existing authentication tokens')
    
    # Show token details command
    details_parser = subparsers.add_parser('details', help='Show detailed information about a specific token')
    details_parser.add_argument('token_id', type=int, help='ID of the token to show details for')
    
    # Search tokens command
    search_parser = subparsers.add_parser('search', help='Search and filter tokens')
    search_parser.add_argument('--name', help='Search by name pattern (case-insensitive)')
    search_parser.add_argument('--description', help='Search by description pattern (case-insensitive)')
    search_parser.add_argument('--active-only', action='store_true', help='Show only active tokens')
    search_parser.add_argument('--inactive-only', action='store_true', help='Show only inactive tokens')
    
    # Update token command
    update_parser = subparsers.add_parser('update', help='Update token name and/or description')
    ...
    deactivate_parser = subparsers.add_parser('deactivate', help='Deactivate an authentication token')
    deactivate_parser.add_argument('token_id', type=int, help='ID of the token to deactivate')
    
    # Reactivate token command
    reactivate_parser = subparsers.add_parser('reactivate', help='Reactivate a deactivated authentication token')
    reactivate_parser.add_argument('token_id', type=int, help='ID of the token to reactivate')
    
    # Analytics command
    analytics_parser = subparsers.add_parser('analytics', help='Show token usage analytics and statistics')
    ...
    export_parser.add_argument('--active-only', action='store_true', help='Export only active tokens')
    
    # Cleanup command
    cleanup_parser = subparsers.add_parser('cleanup', help='Clean up old unused tokens')
    cleanup_parser.add_argument('--days', type=int, default=90, help='Remove tokens older than this many days (default: 90)')
    cleanup_parser.add_argument('--no-dry-run', action='store_true', help='Actually perform cleanup (default is dry run)')
    ...
    if args.command == 'create':
        ...
        asyncio.run(search_tokens(args.name, args.description, args.active_only, args.inactive_only))
